...
async function konsultan(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const response = await axios.get(
        `${API_BACKEND_HOST}/api/v1/user-konsultan`,
        {
          params: {
            tingkat_konsultan: req.query.tingkat_konsultan,
    ...
    try {
      const response = await axios.delete(
        `${API_BACKEND_HOST}/api/v1/user-konsultan/${req.query.id}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
