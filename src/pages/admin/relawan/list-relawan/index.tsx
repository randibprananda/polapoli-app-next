...
import { ModalAddListRelawan } from '../../../../components/organisms';
...
import { useCurrentTeam, useProfile, useRelawan } from '../../../../swr';
import {
  checkIsPremium,
...
const ListRelawanPage = () => {
  ...
  const { data: relawanData, isLoading } = useRelawan(
    refresh,
    currentPage,
    search
  );
  ...
  const onCreate = async (values: any) => {
    const formData = {
      ...values,
      is_saksi: values.is_saksi ? 1 : 0
    };

    setLoading(true);
    // ! Dicomment karena fetchWrapper melempar ke page error
    // fetchWrapper
    //   .post('/api/relawan', formData)
    //   .then(() => {
    //     responseMessage({
    //       type: 'success',
  ...
  const onUpdate = async (values: any) => {
    ...
    // fetchWrapper
    await axios
      .post(`/api/relawan?id=${id}`, formData)
      .then(() => {
        responseMessage({
          type: 'success',
          message: 'Data berhasil ditambahkan',
  ...
  const columns: any = [
    ...
    },
    {
      ...
      width: 200,
      render: (data: any) => {
        if (data.detail_user?.status_invitation?.toLowerCase() == 'active') {
          ...
          <Row gutter={[8, 8]}>
            <RenderIf
              isTrue={checkPermissionArray({
                ...
                idRole: PERMISSION.log_aktifitas_relawan
              })}
            >
              <Col>
                <Button
                  onClick={() =>
                    router.push(`/admin/log-aktivitas/relawan/${record?.id}`)
                  }
  ...
  return (
    <>
      <Head>
        <title>List Relawan | Relawan</title>
      </Head>
      <HeaderPage
      ...
      <Row className="mt-7">
        ...
        {isLoading ? (
          ...
          <>
            <Col xs={24}>
              <Table
                dataSource={relawanData?.data?.data}
                columns={columns}
                scroll={{ x: 1000 }}
                pagination={{
                  total: relawanData?.data?.total,
                  current: currentPage,
...
ListRelawanPage.layout = Admin;

export default ListRelawanPage;
